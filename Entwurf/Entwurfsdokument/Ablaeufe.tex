...
\newline
Danach wird für alle Rasterpunkte entschieden, wie viele und welche 3D-Modelle zur Darstellung des Kantenübergangs benötigt werden. Dabei werden zwischen null und drei NodeModelInfo-Objekte mit teilweise unterschiedlichen Modell-Dateien, Drehungen und Skalierungen erstellt, aus denen mit Hilfe der für \class{NodeModel}-Objekte zuständigen \class{ModelFactory}-Instanz die entsprechenden \class{NodeModel}-Objekte abgefragt bzw. erstellt werden. Diese werden dann, analog zu dem Vorgehen bei den \class{PipeModel}-Objekten, in die \class{NodeModel}-Liste des \class{KnotRenderer}s eingefügt.
