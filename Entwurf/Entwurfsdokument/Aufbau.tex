...
Unser Entwurf basiert auf dem Entwurfsmuster Model View Controller, bei dem die Aufgaben bei einer Anwendung strikt in drei Einheiten aufgeteilt werden. Diese drei Einheiten sind  Datenmodell(Model), Präsentation (View) und Programmsteuerung (Controller). In das Datenmodell werden die Klassen \class{Knot}, \class{Edge} und \class{Circle} eingeteilt, da diese die darzustellenden Daten enthalten und die Präsentation über Änderungen informieren. Die Präsentation zeigt die Daten in grafischer Form an und informiert die Programmsteuerung über Benutzereingaben. Die Präsentation wird von den Klassen \class{PipeModel}, \class{ArrowModel} und \class{NodeModel} repräsentiert.
Die Programmsteuerung kümmert sich um die Interaktion mit dem Benutzer und entspricht der Klasse \class{KnotRenderer}.

\subsection{Kompositum}
Das Kompositum gehört zu der Kategorie der Varianten-Muster und wird in unserem Entwurf von den Klassen \class{KnotRenderer}, \class{EdgeMovement}, \interface{IGameScreenComponent}, \class{Menu}, \class{MenuItem}, \class{ColorPicker} und \class{Dialog} dargestellt.
...
Diese abstrakte Klasse entspricht in unserem Entwurf beispielsweise der Klasse \class{Widget}. Die von \class{Widget} zur Verfügung gestellten Funktionalitäten werden dann von \class{Menu}, \class{MenuItem}, \class{ColorPicker} und \class{Dialog} implementiert und erweitert.

\subsection{Iterator}
Der Iterator gehört zu der Kategorie der Entkopplungsmuster und wird in unserem Entwurf des Öfteren verwendet.
Ein Iterator ist ein spezieller Zeiger, den man verwendet um auf Elemente einer Menge zuzugreifen.
In unserem Entwurf ist beispielsweise \class{Knot} ein Iterator über \class{Edge}s.
