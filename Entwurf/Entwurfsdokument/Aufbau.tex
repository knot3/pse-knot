...
\\\\
Spielkomponenten, die neben der Update()-Methode auch eine Draw()-Methode besitzen, erben von \class{DrawableGameScreenComponent}. Dies sind vor allem die Elemente, aus denen die grafische Benutzeroberfläche zusammengesetzt ist, deren abstrakte Basisklasse \class{Widget} darstellt. [weitere Erklärungen zu Widgets...]
\\\\
...
Dieses Entwurfsmuster kann man in den Unterklassen von \class{Widget} finden. \class{Widget} stellt hierbei die Komponente dar und die Unterklassen \class{Menu}, \class{MenuItem}, \class{ColorPicker} und \class{Dialog} jeweils das Kompositum. 
Desweiteren stellen die Klassen \class{KnotRenderer}, \class{EdgeMovement} und \class{IGameScreenComponent} ein Kompositum dar.
